...
<%
	...
	String strSQL = "Select category,userID,subject,content,regdate,views FROM test_nsz.tb_Board where num="+number+";";
...
<table class="viewTable">
	...
	<tr class="viewTrButton">
		<td colspan="4">
			...
			<input type="button" value="수정" class="goButton" onClick="goWrite('<%=category%>',<%=number%>)"/>
			<%} if(userID.equals("admin")||userID.equals(name)){ %>
			<input type="button" value="삭제" class="goButton" onClick="goDelete('<%=category%>', <%=number%>)"/>
			<%}%>
		</td>
	</tr>
</table>
